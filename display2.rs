...
/// for the `std` library. How shoudl ambiguous types be displayed? For example,
...
// A structure holding two numbers. `Debug` will be derived so the reuslts can
...
impl fmt::Display for Complex {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		// Denote complex number using a + bi format.
...
fn main() {
	...
	println!("The big range is {big} and the small is small {small}",
			 small = small_range,
			 big = big_range);
